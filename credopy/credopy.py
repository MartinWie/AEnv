...
def help():
  ...
  -n do not query credentials at all
...
def getCofigPath():
    ...
    if(path.exists(pydoConfigPath) == True):
        configExists = True
...
def getBotoClients():
    useSession = (os.getenv('AWS_REGION') != None or os.getenv('PROFILENAME') != None)
...
def app():
    ...
    if (os.getenv('CREDO_NO_AWS') == 'true'):
        ...
        if(os.getenv('AWS_REGION') == None and (os.getenv('AWS_REGION') != None or os.getenv('PROFILENAME') != None)):
        ...
        if (os.getenv('OVERRIDE_ENV') != None):
...
def main():
    ...
    if(os.getenv('DEFAULTSERVICE') == None and os.getenv('SERVICE') == None):
